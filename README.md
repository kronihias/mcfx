...
- `mcfx_mimoeq`: **tilt** is also available in the linear-phase FIR designer — the same straight-line-in-dB shape as the IIR band, but realised exactly (the FIR is designed from the target line itself rather than approximated by a filter cascade) and with linear phase. The line is held flat below 20 Hz; the bottom octaves need a long filter to resolve, so check the realised-magnitude preview when using short lengths.
...
- `mcfx_mimoeq`: the display settings have their own row under the mode/path row — analyzer and phase toggles, the spectrum/spectrogram view and its pre/post source, plus a gear for the rest (analyzer channel, auto-normalize, offset). What the graph *shows* is now separated from what is being *edited*, and the two most-switched analyzer options no longer need a popup at all.
