...
- `mcfx_mimoeq`: long FIR bands (over 128 taps) now use safe-mode convolution in hosts that send irregular block sizes (Adobe Audition / Premiere, Steinberg), like `mcfx_convolver` already did; there they produced wrong audio. This adds one block of latency in those hosts, reported to the host. The environment variable `MCFX_CONVOLVER_SAFEMODE=1` (or `0`) forces safe mode on (or off) in both plug-ins, e.g. for another host with irregular blocks.
- `mcfx_mimoeq`: long FIR bands (over 128 taps) reported one host block too much latency (e.g. 1535 instead of 1023 samples for 2047 taps at a 512 block), so hosts over-compensated by a block. The reported latency is now the FIR's group delay, matching the actual delay and the FIR designer's figure.
- `mcfx_mimoeq`: a disabled FIR (or lookahead) band no longer counts toward the reported latency. Switching such a band on or off, also by automation, now updates the reported latency and the internal channel alignment right away; before, the old latency stayed reported and the other channels stayed delayed to match it.
- `mcfx_graph`: **per-node "Ignore latency"** (right-click a node) leaves that node out of delay compensation, e.g. a lookahead plug-in in one branch that shouldn't delay every other branch in live use. Node tiles now show their latency, in amber when ignored.
