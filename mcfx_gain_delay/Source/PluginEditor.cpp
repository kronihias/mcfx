    ...
    : AudioProcessorEditor (ownerFilter)
{
    ...
    if (NUM_CHANNELS > GROUP_CHANNELS)
        setSize (505, 60+NUM_CHANNELS/2*LINE_WIDTH + METER_GROUP_SPACE * NUM_CHANNELS/GROUP_CHANNELS);
    else
        setSize (280, 60+NUM_CHANNELS*LINE_WIDTH + METER_GROUP_SPACE*NUM_CHANNELS/GROUP_CHANNELS);
...
void Mcfx_gain_delayAudioProcessorEditor::sliderValueChanged (Slider* sliderThatWasMoved)
{
    ...
    if (param_nr == 0)
        ourProcessor->setParameterNotifyingHost(5*ch_nr+param_nr, (float)sliderThatWasMoved->getValue()/36.f+0.5f);
    else
        ourProcessor->setParameterNotifyingHost(5*ch_nr+param_nr, (float)sliderThatWasMoved->getValue()/(MAX_DELAYTIME_S*1000.f));
...
void Mcfx_gain_delayAudioProcessorEditor::buttonClicked (Button* buttonThatWasClicked)
{
    ...
    { // channel setting button pressed... (phase, mute, solo)
        ...
        if (btn_nr < NUM_CHANNELS)
        {
            ...
            ourProcessor->setParameterNotifyingHost(ch_nr*5+2, val);
        }
        // mute button pressed
        else if (btn_nr < 2*NUM_CHANNELS)
        {
            ...
            ourProcessor->setParameterNotifyingHost(ch_nr*5+3, val);
        }
        // solo button pressed
        else
        {
            ...
            ourProcessor->setParameterNotifyingHost(ch_nr*5+4, val);
        }
...
void Mcfx_gain_delayAudioProcessorEditor::changeListenerCallback (ChangeBroadcaster *source)
{
    ...
    for (int i=0; i < NUM_CHANNELS; i++)
    {
        sld_del.getUnchecked(i)->setValue(ourProcessor->getParameter(5*i+1) * MAX_DELAYTIME_S*1000,dontSendNotification);
        
        sld_gain.getUnchecked(i)->setValue((ourProcessor->getParameter(5*i)-0.5f)*36.f,dontSendNotification);
      
        if (ourProcessor->getParameter(5*i+2) > 0.5) // phase
        {
          String tooltip = "ch";
          tooltip.append(String(i+1), 3);
        ...
        if (ourProcessor->getParameter(5*i+3) > 0.5) // mute
        {
            String tooltip = "ch";
            tooltip.append(String(i+1), 3);
            tooltip << " muted";
        ...
        if (ourProcessor->getParameter(5*i+4) > 0.5) // solo
        {
            String tooltip = "ch";
            tooltip.append(String(i+1), 3);
            tooltip << " soloed";
