...
void Mcfx_gain_delayAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
  ...
  // until the end of the ring" count then goes negative and the copy runs off
  // the buffer. Hosts and validators re-prepare one instance at several rates
  // in a row (Steinberg's ProcessFormatTest does exactly this; same crash as
  // mcfx_delay's under Isadora's plug-in scan).
